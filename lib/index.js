...
module.exports = class Microinvoice {
  ...
  generateTableRow(type, columns) {
    ...
    columns.forEach((column, index) => {
      ...
      if (column.price === true) {
        _value = this.prettyPrice(_value, column?.digits);
      }
  ...
  generateParts() {
    ...
    (this.options.data.invoice.details.total || []).forEach((total) => {
      ...
      let _primaryColorCode = total.highlight ? 'highlight' : 'primary';
      let _secondaryColorCode = total.highlight ? 'highlight' : 'secondary';
      let _link = total.link ? total.link : null;
      this.setCursor("x", this.options.style.total.label.position);
      this.setText(total.label, {
        colorCode: _primaryColorCode,
        fontWeight: "bold",
        marginTop: 12,
   ...
   */
  generateButton(){
    if(this.options.data.invoice.details.button){
      this.storage.cursor.y += 25;
      const _bgColor = this.options.data.invoice.details.button.bgColor || 'yellow';
      const _textColor = this.options.data.invoice.details.button.textColor || 'red';
      const _label = this.options.data.invoice.details.button.label
      const _link = this.options.data.invoice.details.button.value
      this.document.fontSize(25)
      .highlight(this.options.style.total.label.position, this.storage.cursor.y + 5, this.document.widthOfString(_label), 30, { color : _bgColor })
      .text(_label, this.options.style.total.label.position, this.storage.cursor.y, { color : _textColor, link : _link });
    }
  ...
  setTextStyle(text, options) {
    ...
    if (!_color) {
      ...
      } else if(_colorCode === "secondary"){
        this.document.fillColor(this.options.style.text.secondaryColor);
      } else {
        this.document.fillColor(this.options.style.text.highlightColor);
      }
    }
